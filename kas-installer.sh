...
read_kas_installer_env_file() {
  ...
  KAS_FLEET_MANAGER_SERVICE_TEMPLATE_PARAMS="${KAS_FLEET_MANAGER_SERVICE_TEMPLATE_PARAMS:-}"
...
generate_kas_fleet_manager_env_config() {
  ...
  echo "STRIMZI_OPERATOR_SUBSCRIPTION_CONFIG='${STRIMZI_OPERATOR_SUBSCRIPTION_CONFIG}'" >> ${KAS_FLEET_MANAGER_DEPLOY_ENV_FILE}
  ...
  echo "KAS_FLEETSHARD_OPERATOR_SUBSCRIPTION_CONFIG='${KAS_FLEETSHARD_OPERATOR_SUBSCRIPTION_CONFIG}'" >> ${KAS_FLEET_MANAGER_DEPLOY_ENV_FILE}
  ...
  # Special variable itself potentially containing multiple variables. Write a heredoc to preserve structure
  printf "KAS_FLEET_MANAGER_SERVICE_TEMPLATE_PARAMS=\"\$(cat <<EOF\n%s\nEOF\n)\"\n" "${KAS_FLEET_MANAGER_SERVICE_TEMPLATE_PARAMS}" >> ${KAS_FLEET_MANAGER_DEPLOY_ENV_FILE}
